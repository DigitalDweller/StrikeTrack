...
export default function ScanScreen() {
  ...
  const processImage = async (uri: string) => {
    ...
    try {
      ...
      setEditedCharge(
        p.chargePercent != null
          ? String(Math.round(clampChargePercent(p.chargePercent)))
          : ''
      );
      setEditedV0(
  ...
  const handleSave = async () => {
    const charge = clampChargePercent(
      parseFloat(editedCharge.replace(',', '.')) || 0
    );
    await insertReading({
      ...
      >
        ...
        <View style={styles.field}>
          ...
          <TextInput
            style={styles.input}
            value={editedCharge}
            onChangeText={(v) => setEditedCharge(capChargePercentInput(v))}
